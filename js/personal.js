(function () {
  ...
  // ─────────────────────────────────────────────────────────────
  // PERSONAL CARE — subskills & drills
  // ─────────────────────────────────────────────────────────────
  const TIERS = [
    { key: 'pre',  label: 'Preschool Age',      color: '#1447e6', cls: 't-eye'   },
    { key: 'elem', label: 'Elementary Age',      color: '#00c950', cls: 't-basic' },
    { key: 'adol', label: 'Pre-adolescent Age',  color: '#facc15', cls: 't-ident' },
  ];

  const DRILL_OPTIONS = {
    'Preschool Age':     ['Removing Shoes', 'Washing Hands'],
    'Elementary Age':    ['Front or Back', 'Tying Shoelace'],
    'Pre-adolescent Age':['Healthy or Not Healthy', 'Match the Picture'],
  };

  // Auto-attempts per drill
  const DRILL_ATTEMPTS = {
    'Removing Shoes':        4,
    'Washing Hands':         7,
    'Front or Back':         5,
    'Tying Shoelace':        7,
    'Healthy or Not Healthy':4,
    'Match the Picture':     4,
  };

  // All drills in Personal Care use "points"
  const DRILL_UNITS = {
    'Removing Shoes':        'points',
    'Washing Hands':         'points',
    'Front or Back':         'points',
    'Tying Shoelace':        'points',
    'Healthy or Not Healthy':'points',
    'Match the Picture':     'points',
  };

  function getAttempts(drillName) { return DRILL_ATTEMPTS[drillName] ?? 5; }
  function getUnit(drillName)     { return DRILL_UNITS[drillName]    || 'attempts'; }
  ...
  function luminance(hex) {
    const [r, g, b] = hexToRgb(hex).split(',').map(v => {
      const n = +v / 255;
  ...
  function makeBadgeHTML(label, color) {
    ...
    const textColor = luminance(color) > 0.35 ? '#5a4500' : color;
    return (
      '<span class="cs-badge-label" style="' +
        'color:' + textColor + ';' +
        'background:rgba(' + rgb + ',0.12);' +
        'border-color:rgba(' + rgb + ',0.35);">' +
        '<span class="cs-badge-dot" style="background:' + color + '"></span>' +
        label +
      '</span>'
    );
  }
  ...
  function pollForClient() {
    if (window.ATRP_CLIENT_ID) { resolveClient(); }
    else if (_pollCount < 20)  { _pollCount++; setTimeout(pollForClient, 150); }
    else                       { updateClientBanner(); }
  }
  ...
  function updateClientBanner() {
    if (!document.getElementById('_skillBannerStyles')) {
      ...
      s.textContent = `
        #skillClientBanner{display:flex;align-items:center;justify-content:space-between;padding:14px 22px;background:linear-gradient(135deg,#fdfaf4 0%,#faf7ed 100%);border-bottom:1.5px solid rgba(201,168,76,.18);font-family:var(--ff-body);gap:12px;}
  ...
  function rebuildDrillPanel(idx, tierKey) {
    ...
    // Reset drill, attempts, unit
    DRILLS[idx].skill    = '';
    DRILLS[idx].attempts = 0;
    DRILLS[idx].unit     = 'attempts';

    const drillLabelWrap = document.getElementById('drill-label-'+idx);
    if (drillLabelWrap) {
      drillLabelWrap.innerHTML   = '';
      drillLabelWrap.textContent = 'Select drill…';
      drillLabelWrap.classList.add('placeholder');
    }
    const attEl  = document.getElementById('attempts-' + idx);
    const unitEl = document.getElementById('unit-' + idx);
  ...
  // ─────────────────────────────────────────────────────────────
  // SUMMARY — 3-column grid for 3 tiers
  // ─────────────────────────────────────────────────────────────
  function renderSummary() {
    ...
    // Force 3-column layout for this page
    ...
    TIERS.forEach(t => {
      ...
      // Determine text color for the tier badge in summary card
      const rgb       = hexToRgb(t.color);
      const lum       = luminance(t.color);
      const textColor = lum > 0.35 ? '#5a4500' : t.color;

      const card = document.createElement('div');
      card.className = 'tier-card ' + t.cls + (inactive ? ' inactive' : '');
      card.innerHTML =
        '<div class="tc-top">'+
          '<span class="tc-badge" style="background:'+t.color+';color:'+(lum>0.35?'#3d2b00':'#fff')+'">'+
            '<span class="tc-badge-dot" style="background:rgba('+(lum>0.35?'0,0,0':'255,255,255')+',.5)"></span>'+
            t.label+
          '</span>'+
        ...
        (inactive || pct === null
          ...
          : '<div class="tc-stat">'+
            ...
            '</div>'+
            '<span class="tc-fraction">'+a.totalSucc+' / '+a.totalAttempts+' pts</span>'
        )+
        '<div class="tc-bar-track">'+
          '<div class="tc-bar-fill" style="width:'+(pct||0)+'%;background:'+t.color+'"></div>'+
        '</div>'+
        '<div class="tc-drills">'+
          (a.drills.length === 0
            ? '<span class="tc-drill-tag unnamed">No drills assigned</span>'
            : a.drills.map(n =>
                n ? '<span class="tc-drill-tag">'+n+'</span>'
  ...
  function buildRow(drill, idx) {
    ...
    // ── Subskill trigger label ──
    const subLabelHTML = tier
      ? makeBadgeHTML(tier.label, color)
      : '<span class="cs-label placeholder">Select subskill…</span>';
    ...
    const drillLabelHTML = (selectedDrill && color)
      ...
      : '<span class="cs-label placeholder">Select drill…</span>';
    ...
    // 3. Attempts — readonly, auto-filled
